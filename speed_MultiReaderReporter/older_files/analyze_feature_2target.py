...
def main(dir_path: Path, out_dir: Path):
    ...
    # Keep interpolated trajectories (built with target_soh_plot)
    ...
    for csv_file in dir_path.glob("*.csv"):
        ...
        # --- build interpolated curve for plotting (uses target_soh_plot) ---
        interpolated_plot = load_and_interpolate(df_filter, target_soh_plot, interpolation_typ="weeks")
        if interpolated_plot is None or interpolated_plot.empty:
            print(f"[WARN] {cell_name}: interpolation failed, skipping.")
            continue

        if "SOH" not in interpolated_plot.columns or "weeks" not in interpolated_plot.columns:
            print(f"[WARN] {cell_name}: interpolated_df missing SOH/weeks, skipping.")
            continue

        traj_by_cell[cell_name] = interpolated_plot[["weeks", "SOH"]].copy()
        ...
        idx_feat = (interpolated_plot["SOH"] - target_soh_features).abs().idxmin()
        row_feat = interpolated_plot.loc[idx_feat]
    ...
    # Finalize dataframes (built at target_soh_features)
    df_ref = pd.DataFrame(rows_ref).reset_index(drop=True)
    df_exp = pd.DataFrame(rows).reset_index(drop=True)

    # -----------------------------
    # Explicitly plot the 3 references again (visible on top)
    # -----------------------------
    ...
    if ref_sub.empty:
        ...
        print(f"\nClosest {len(closest_cellnames)} exp cells to MEAN(refs) at SOH={target_soh_features}:")
        for cn in closest_cellnames:
            print(" -", cn)

        print(f"\nFarthest {len(farthest_cellnames)} exp cells from MEAN(refs) at SOH={target_soh_features}:")
        for cn in farthest_cellnames:
            print(" -", cn)

        # 2D scatter in its own figure (do not show here; show once at the end)
        ...
        ax_sc.scatter(
            ...
            edgecolors="k",
            linewidths=0.6,
            label=f"closest {k1} to mean(ref)",
        )
        ax_sc.scatter(
            ...
            edgecolors="k",
            linewidths=0.6,
            label=f"farthest {k2} from mean(ref)",
        )

        # show mean reference point
        ax_sc.scatter(
            [ref_mean[0]],
            [ref_mean[1]],
            s=130,
    ...
    # (trajectories themselves are interpolated using target_soh_plot)
    # -----------------------------
    ...
    ax.set_ylabel("SOH")
    ax.set_title(
        f"SOH trajectories (plot interpolation target SOH={target_soh_plot}; "
        f"feature comparison at SOH={target_soh_features})"
    )
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best")

    # Show ALL figures once (prevents blank last figure)
    fig.tight_layout()
    plt.show()

    # Tag df_exp for later use (optional)
    df_exp = df_exp.copy()
    df_exp["is_closest_to_mean_ref_by_2d"] = df_exp["cell_name"].isin(closest_cellnames)
    df_exp["is_farthest_from_mean_ref_by_2d"] = df_exp["cell_name"].isin(farthest_cellnames)

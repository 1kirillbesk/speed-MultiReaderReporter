...
def extract_features(df,cell,cfg):
    ...
    # CU_num = [17, 2, 6, 9]
    choice = int((df.loc[df["step_int"] == 57, "current_A"] < 0).any())
    rpt_mask = (df["procedure"].str.contains("Digi", case=False, na=False))

    ocv_cha_step = cfg['CU_steps']['ocv_cha'][choice]; ocv_dis_step = cfg['CU_steps']['ocv_dis'][choice]
    capa_cha_step = cfg['CU_steps']['capa_cha'][0]; capa_dis_step = cfg['CU_steps']['capa_dis'][0]
    ...
    mask_capa_dis = ((df["step_int"] == capa_dis_step) & rpt_mask)

...
def extract_features(df,cell,cfg):
    ...
    # CU_num = [17, 2, 6, 9]
    mask_ocv_cha = ((df["step_int"] == cfg['CU_steps']['ocv_cha']) &(df["procedure"].str.contains("rpt", case=False, na=False)))
    mask_ocv_dis = ((df["step_int"] == cfg['CU_steps']['ocv_dis']) &(df["procedure"].str.contains("rpt", case=False, na=False)))
    mask_capa_cha = ((df["step_int"] == cfg['CU_steps']['capa_cha']) & (df["procedure"].str.contains("rpt", case=False, na=False)))
    mask_capa_dis = ((df["step_int"] == cfg['CU_steps']['capa_dis']) & (df["procedure"].str.contains("LWcp", case=False, na=False)))

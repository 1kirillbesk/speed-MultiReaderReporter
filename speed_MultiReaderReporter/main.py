...
def main():
    ...
    cell_names = cfg.get("input", {}).get("cell_name_contain", [])
    if cell_names:
        cell_patterns = [
            # '=' is the field delimiter in these filenames, so it ends a cell id
            # just like '_' does; without it a full id ("SPEED_LWHK_cycle_26")
            # would never match while a prefix ("SPEED_LWHK") would.
